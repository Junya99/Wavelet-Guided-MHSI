...
def main(args):
    ...
    history = {'test_iou':[], 'test_dice':[], 'test_hausf':[], 'test_iou_var':[], 'test_dice_var':[], 'test_hausf_var':[],
               'inference_time':[], 'Params':[], 'params_M':[], 'MACs_G':[]}


    # record model parameter count in the evaluation CSV.
    params = sum(p.numel() for p in model.parameters())
    history['Params'].append(params)
    history['params_M'].append(params / 1e6)
    history['MACs_G'].append(0.0)#macs
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pretrained_model', '-pm', type=str,
                        default='./checkpoints/best_dice0.7646.pth')
